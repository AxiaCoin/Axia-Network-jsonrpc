...
fn serve<F: FnOnce(ServerBuilder) -> ServerBuilder>(alter: F) -> Server {
	let builder = ServerBuilder::new(io())
		.cors(DomainsValidation::AllowOnly(vec![
			AccessControlAllowOrigin::Value("axiacoin.network".into()),
...
fn should_add_cors_allow_origins() {
	...
	assert!(
		response
			.headers
			.contains("access-control-allow-origin: http://axiacoin.network"),
		"Headers missing in {}",
		response.headers
	);
}
...
fn should_reject_invalid_hosts() {
	// given
	let server = serve_hosts(vec!["axiacoin.network".into()]);
