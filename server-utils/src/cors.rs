...
mod tests {
	...
	fn should_parse_origin() {
		...
		assert_eq!(Origin::parse("http://axiacoin.network"), Origin::new(Http, "axiacoin.network", None));
		assert_eq!(
			Origin::parse("https://axiacoin.network:8443"),
			Origin::new(Https, "axiacoin.network", Some(8443))
		);
		assert_eq!(
		...
		assert_eq!(
			Origin::parse("axiacoin.network/somepath"),
			Origin::new(Http, "axiacoin.network", None)
		);
		assert_eq!(
	...
	fn should_return_domain_when_all_are_allowed() {
		// given
		let origin = Some("axiacoin.network");
		let host = None;
		...
		// then
		assert_eq!(res, AllowCors::Ok("axiacoin.network".into()));
	}
	...
	fn should_return_none_for_not_matching_origin() {
		// given
		let origin = Some("http://axiacoin.network".into());
		let host = None;
	...
	fn should_return_specific_origin_if_we_allow_any() {
		...
		assert_eq!(
			res,
			AllowCors::Ok(AccessControlAllowOrigin::Value("http://axiacoin.network".into()))
		);
	}
	...
	fn should_return_specific_origin_if_there_is_a_match() {
		...
		let res = get_cors_allow_origin(
			...
			&Some(vec![
				...
				AccessControlAllowOrigin::Value("http://axiacoin.network".into()),
			]),
		);
	...
	fn should_support_wildcards() {
		// given
		let origin1 = Some("http://axiacoin.network".into());
		let origin2 = Some("http://axiacoin.networkt".into());
